...
{    
    ...
    new->LMI_FIFO_Watermark = I810CalcWatermark(modetiming->pixelClock/1000,
			modeinfo->bitsPerPixel, lmfreq, 0);
...
{   
	...
	if(!__svgalib_i810_usegtt) offset>>=1;
...
{
    if (op==LINEAR_ENABLE || op==LINEAR_DISABLE) return 0;
            ...
            switch(p2) {
                case 0:
                    ...
                    for(j=0;j<16;j++) {
                        ...
                        *(unsigned int *)(LINEAR_POINTER+i+16*j+4)=l4;
                        *(unsigned int *)(LINEAR_POINTER+i+16*j+8)=l1;
                        *(unsigned int *)(LINEAR_POINTER+i+16*j+12)=l3;
                    }
                break;
            }
            break;
...
static int init(int force, int par1, int par2)
{
	...
	if(__svgalib_i810_usegtt) {
		...
		__svgalib_i810_driverspecs.__svgalib_setpage = NULL;	
	} else {
    	linear_base=0;
